...
tests_work_queue :: proc(t: ^testing.T) {
	...
	assert(M <= N)
	for i in 0 ..< M {
		threads.append_work(&threads.work_queue, threads.Work{procedure = work_1, data = &checksum})
		threads.append_work(&threads.work_queue, threads.Work{procedure = work_1, data = &checksum})
		threads.append_work(&threads.work_queue, threads.Work{procedure = work_2, data = &checksum})
	}
