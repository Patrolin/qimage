...
updateAndRender :: proc() {
	...
	startPos := inputs.mouse.pos.buffer[0]
	fmt.println("startPos", startPos)
	if startPos.x >= 0 && startPos.y >= 0 {
		for y := 0; y < int(image.height) && (int(startPos.y)+y) < int(frame_buffer.height); y += 1 {
			for x := 0; x < int(image.width) && (int(startPos.x)+x) < int(frame_buffer.width); x += 1 {
				R := image.data[y * stride + x * pitch]
				G := image.data[y * stride + x * pitch + 1]
				B := image.data[y * stride + x * pitch + 2]
				paint.packRGBA(frame_buffer, int(startPos.x) + x, int(startPos.y) + y, math.v4i{
